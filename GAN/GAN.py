...
train_images = train_images.astype('float32') #转换为浮点数
...
datasets = datasets.shuffle(BUFFER_SIZE).batch(BATCH_SIZE)  #打乱图片的顺序

#做一个生成器模型
def generator_mode():

    model = keras.Sequential()
    model.add(layers.Dense(256,input_shape=(100,),use_bias=False))  #输入噪声数据
    ...
    model.add(layers.Dense(28*28*1,use_bias=False,activation='tanh'))  #输出28*28图片
    model.add(layers.BatchNormalization())

    model.add(layers.Reshape((28,28,1)))  #reshape 为28*28*1图片
...
def discriminator_model():
    ...
    model.add(layers.Dense(1))   #输出一维的结果 真1，假 0
